        ...
        <div class="wu-toolbar-search" >
            ...
            <select id="s_sdkId" class="easyui-combobox">
                ...
                <c:forEach items="${sdkList}" var="item">
                    <option value="${item.sdkId}">${item.cch.cchName}[${item.cch.cchId}]</option>
                </c:forEach>
            </select>
            <label>系统版本：</label>
            <select id="s_sdkSystem" class="easyui-combobox">
                <option value="">全部</option>
    ...
    <form id="wu-form-1" method="post"  enctype="multipart/form-data">
        <table style="border-collapse:separate; border-spacing:0 6px;">
            <tr>
                ...
                <td>
                    <select name="cch.id" class="easyui-combobox"
...
<script type="text/javascript">
    ...
    function add() {
        $('#wu-form-1').form('submit', {
            url: '${ctx}/sdk/sdkOverseaManager/add',
    ...
    function edit() {
        $('#wu-form-2').form('submit', {
            url: '${ctx}/sdk/sdkOverseaManager/edit',
            success: function (data) {
                // 隐藏
                disLoad();
...
//            $.messager.alert("信息提示", "请先选择修改的数据项", "info");
    ...
    function change() {
        $('#wu-form-3').form('submit', {
            url: '${ctx}/sdk/sdkOverseaManager/change',
            success: function (data) {
                // 隐藏
                disLoad();
    ...
    function loadData(query_params) {
        ...
        $('#wu-datagrid-2').datagrid({
            idField: 'sdkId',
            url: '${ctx}/sdk/sdkOverseaManager/list',
            ...
            columns: [[
                {field: 'cch.cchId', title: '渠道ID', width: 70,
                    formatter: function (val, row) {return row.cch.cchId;}},
                {field: 'cch.cchName', title: 'SDK名称', width: 140,
                    formatter: function (val, row) {return row.cch.cchName;}},
                {field: 'sdkSystem', title: '系统版本', width: 80, sortable: true, hidden: true,
                ...
                {field:'operation2', title: '操作2', width: 80, formatter: function (val, row) {
                    return '<a class="wu-table-operation"  href="sdkVersionManager?sdkId=' + row.sdkId + '">查询详情</a>';
                }},
    ...
    function startSearch() {
        ...
        var sdkSystem = $('#s_sdkSystem').combobox('getValue');
