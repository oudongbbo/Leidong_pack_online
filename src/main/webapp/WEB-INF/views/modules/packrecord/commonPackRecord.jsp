...
<script type="text/javascript">
    ...
    function loadData(query_params) {
        ...
        $('#wu-datagrid-2').datagrid({
            idField: 'apkChangeId',
            url: '${ctx}/packRecord/commonPackRecord/list',
            ...
            fit: true,
            columns: [[
                {field: 'apkChangeFileSource', title: '母包来源', width: 70,
                ...
                {field: 'apkChangeGameId', title: '换游戏Id', width: 100, sortable:true},
                {field: 'apkChangeCchId', title: '换渠道Id', width: 100, sortable:true},
                {field: 'apkChangeMdId', title: '换媒体Id', width: 100, sortable:true},
                {field: 'apkChangeName', title: '换游戏名', width: 100},
                {field: 'apkChangePackage', title: '换包名', width: 180},
                {field: 'apkChangeVersionNo', title: '换版本号', width: 100, sortable:true},
                {field: 'apkChangeVersionName', title: '换版本名', width: 100},
                {field: 'apkChangeIcon', title: '换Icon', width: 50,
                    formatter: function (val, row) {
                        if (val == 1) {
                            return '有';
                        }
                    }},
                {field: 'apkChangeSign', title: '重新签名', width: 60,
                    formatter: function (val, row) {
                        if (val == 1) {
                            return '是';
                        }
                ...
                {field: 'apkChangeStartDate', title:'开始时间', width:130, sortable:true,
                    formatter: function (val, row) {return toDate(val);}},
                {field: 'apkChangeEndDate', title:'完成时间', width:130, sortable:true,
                    formatter: function (val, row) {return toDate(val);}},
                {field: 'apkChangeInput', title: '母包', width: 70,
                    formatter: function (val, row) {
                        if (val) {
                ...
                {field: 'apkChangeOutput', title: '结果包', width: 70,
                    formatter: function (val, row) {
                        if (val) {
                            return '<a class="wu-table-operation" href="' + val + '">点击下载</a>';
                        } else {
                ...
                {field:'apkChangeResultMsg', title:'打包结果'}
            ]],
